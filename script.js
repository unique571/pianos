...
let allKeys = [],
audio = new Audio('tunes/a.wave'); // by default, audio src is "a" tune


const playTune = (key) => {
    audio.src = `tunes/${key}.wave`; // passing audio src based on key pressed 
    audio.play(); // playing audio
    const clickedKey = document.querySelector(`[data-key="${key.toString()}"]`);
